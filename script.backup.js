import { auth, db, screenDb, signInWithEmailAndPassword, onAuthStateChanged, signOut, ref, onValue, remove, update, push, set, serverTimestamp, off, get, query, limitToLast, orderByKey, endAt, onChildAdded, onChildChanged } from './firebase-config.js';
...
const sendNotificationFreeBtn = document.getElementById('send-notification-free-btn'); // Free Users Button
const sendSingleUserNotificationBtn = document.getElementById('send-single-user-notification-btn');
const lockStatusOverviewBtn = document.getElementById('lock-status-overview-btn');
const lockStatusModal = document.getElementById('lock-status-modal');
const manageTrialsBtn = document.getElementById('manage-free-trials-btn');
...
const globalLockLocationCheckbox = document.getElementById('global-lock-location');
const globalLockPhotoCheckbox = document.getElementById('global-lock-photo');
const globalLockVideoCheckbox = document.getElementById('global-lock-video');
const globalLockAudioCheckbox = document.getElementById('global-lock-audio');
const globalLockScreenshotCheckbox = document.getElementById('global-lock-screenshot');
const globalLockContactsCheckbox = document.getElementById('global-lock-contacts');
const globalLockToggleAppVisibilityCheckbox = document.getElementById('global-lock-toggleAppVisibility');
const globalLockSendSmsCheckbox = document.getElementById('global-lock-sendSms');
const globalLockDeviceStatusCheckbox = document.getElementById('global-lock-deviceStatus');
...
const lockLocationCheckbox = document.getElementById('lock-location');
const lockPhotoCheckbox = document.getElementById('lock-photo');
const lockVideoCheckbox = document.getElementById('lock-video');
const lockAudioCheckbox = document.getElementById('lock-audio');
const lockScreenshotCheckbox = document.getElementById('lock-screenshot');
const lockContactsCheckbox = document.getElementById('lock-contacts');
const lockToggleAppVisibilityCheckbox = document.getElementById('lock-toggleAppVisibility');
const lockSendSmsCheckbox = document.getElementById('lock-sendSms');
const lockDeviceStatusCheckbox = document.getElementById('lock-deviceStatus');
            ...
            for (const uid of Object.keys(users)) {
                ...
                Object.keys(userChildren).forEach(childKey => {
                    ...
                    if (childData && typeof childData === 'object') {
                        ...
                        if (childData.lockedFeatures) {
                            Object.values(childData.lockedFeatures).forEach(lf => {
                                if (lf && lf.isLocked === true) hasLockedFeatures = true;
                            });
                        }
                    }
                });
...
const limitFileScanInput = document.getElementById('limit-filescan');
const limitScreenShareInput = document.getElementById('limit-screenshare');
...
let adminSelectedDuration = 1; // Default: 1 Month (1M)
...
document.querySelectorAll('.plan-dur-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        adminSelectedDuration = parseInt(btn.dataset.months);
        // Update info text
        const infoEl = document.getElementById('duration-info-text');
        if (infoEl) {
            if (adminSelectedDuration === 1) {
                infoEl.textContent = '';
            } else {
                const bDays = adminSelectedDuration * 15;
                const gDays = adminSelectedDuration * 30;
                infoEl.textContent = `Basic=${bDays}d | Gold/Plat=${gDays}d`;
            }
        }
    });
});
...
// Event Listener for Free/Trial Users Notification
if (sendNotificationFreeBtn) {
    sendNotificationFreeBtn.addEventListener('click', () => {
        notificationTarget = { type: 'free' };
        document.getElementById('notification-modal-title').textContent = 'Notify Free/Trial Users 🚨';
        openModal(notificationModal);
    });
}
...
// ==========================================
// LOCK STATUS OVERVIEW (Option C)
// ==========================================
if (lockStatusOverviewBtn) {
    lockStatusOverviewBtn.addEventListener('click', async () => {
        if (!lockStatusModal) return;
        openModal(lockStatusModal);

        const tableBody = document.getElementById('lock-status-table-body');
        const lockedCountEl = document.getElementById('ls-locked-count');
        const trialCountEl = document.getElementById('ls-trial-count');
        const totalCountEl = document.getElementById('ls-total-count');
        const filterSelect = document.getElementById('ls-filter');
        const downloadPdfBtn = document.getElementById('ls-download-pdf');

        tableBody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:30px; color:#bdc3c7;">⏳ Scanning free users...</td></tr>';
        lockedCountEl.textContent = '...';
        trialCountEl.textContent = '...';
        totalCountEl.textContent = '...';

        try {
            const userSnapshot = await get(ref(db, 'user'));
            if (!userSnapshot.exists()) {
                tableBody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:30px; color:#e74c3c;">No users found.</td></tr>';
                return;
            }

            const users = userSnapshot.val();
            const allRows = [];
            let lockedCount = 0, trialCount = 0, totalDevices = 0;
            ...
            for (const uid of Object.keys(users)) {
                ...
                if (userData.profile && userData.profile.email) {
                    ...
                    for (const ck of Object.keys(userData)) {
                        if (['profile', 'subscription', 'config', 'adminNotifications'].includes(ck)) continue;
                        const cd = userData[ck];
                        if (cd && typeof cd === 'object' && cd.email) {
                            userEmail = cd.email;
                            break;
                        }
                    }
                ...
                // Skip paid users at user-level
                let userLevelPaid = false;
                if (userData.subscription) {
                    const uPlan = (userData.subscription.planId || '').toLowerCase();
                    if (['basic', 'gold', 'platinum'].includes(uPlan)) userLevelPaid = true;
                }
                if (userLevelPaid) continue;

                for (const childKey of Object.keys(userData)) {
                    if (['profile', 'subscription', 'config', 'adminNotifications'].includes(childKey)) continue;
                    const childData = userData[childKey];
                    if (!childData || typeof childData !== 'object') continue;
                    if (!childData.deviceStatus && !childData.data && !childData.lockedFeatures && !childData.firstSeen && !childData.subscription) continue;

                    // Skip device-level paid
                    if (childData.subscription) {
                        const dPlan = (childData.subscription.planId || '').toLowerCase();
                        if (['basic', 'gold', 'platinum'].includes(dPlan)) continue;
                    }

                    totalDevices++;
                    let deviceName = childData.nameChild || childKey;
                    let status = 'trial';
                    let statusLabel = '🟡 Trial Active';
                    let statusClass = 'trial';
                    let trialStart = 'N/A';
                    let lockedFeaturesCount = 0;

                    // Check if locked
                    let hasLock = false;
                    if (childData.lockedFeatures) {
                        Object.values(childData.lockedFeatures).forEach(lf => {
                            if (lf && lf.isLocked === true) {
                                hasLock = true;
                                lockedFeaturesCount++;
                            }
                        });
                    ...
                    if (childData.firstSeen) {
                        const dt = new Date(childData.firstSeen);
                        trialStart = dt.toLocaleDateString() + ' ' + dt.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});
                    }
                    ...
                    allRows.push({
                        ...
                        lockedFeaturesCount: lockedFeaturesCount,
                        uid: uid,
                        childKey: childKey
                    });
                }
            ...
            totalCountEl.textContent = totalDevices;

            const renderTable = (filter) => {
                const filtered = filter === 'all' ? allRows : allRows.filter(r => r.status === filter);
                if (filtered.length === 0) {
                    tableBody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px; color:#7f8c8d;">No users found for this filter.</td></tr>';
                    return;
                }
                tableBody.innerHTML = filtered.map(r => {
                    const statusColor = r.statusClass === 'locked' ? '#e74c3c' : '#f1c40f';
                    const bgColor = r.statusClass === 'locked' ? 'rgba(231,76,60,0.1)' : 'rgba(241,196,15,0.1)';
                    return `<tr style="border-bottom: 1px solid #333;">
                        <td style="padding:10px; font-size:0.85rem; max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${r.email}</td>
                        <td style="padding:10px; font-size:0.85rem;">${r.device}</td>
                        <td style="padding:10px; text-align:center;"><span style="background:${bgColor}; color:${statusColor}; padding:4px 10px; border-radius:12px; font-size:0.8rem; font-weight:bold; white-space:nowrap;">${r.statusLabel}</span></td>
                        <td style="padding:10px; text-align:center; font-size:0.8rem; color:#95a5a6;">${r.trialStart}</td>
                        <td style="padding:10px; text-align:center; font-size:0.85rem;">${r.statusClass === 'locked' ? '<span style="color:#e74c3c; font-weight:bold;">' + r.lockedFeaturesCount + '/13</span>' : '<span style="color:#f1c40f;">All Open</span>'}</td>
                    </tr>`;
                }).join('');
            };
            ...
            if (filterSelect) {
                filterSelect.onchange = () => renderTable(filterSelect.value);
            }
            ...
            if (downloadPdfBtn) {
                downloadPdfBtn.onclick = () => {
                    try {
                        ...
                        doc.text('Lock Status Report', margin, y);
                        y += 10;
                        ...
                        doc.text('Total Free: ' + totalDevices + '  |  Locked: ' + lockedCount + '  |  Trial: ' + trialCount, margin, y);
                        y += 12;
                        ...
                        doc.text('Email', margin + 2, y + 6);
                        doc.text('Device', margin + 65, y + 6);
                        doc.text('Status', margin + 110, y + 6);
                        doc.text('Trial Start', margin + 140, y + 6);
                        y += 12;
                        ...
                        allRows.forEach((r, i) => {
                            ...
                            const statusText = r.statusClass === 'locked' ? 'LOCKED' : 'TRIAL';
                            doc.setFontSize(8);
                            doc.text((r.email || '').substring(0, 30), margin + 2, y);
                            doc.text((r.device || '').substring(0, 20), margin + 65, y);
                            
                            if (r.statusClass === 'locked') doc.setTextColor(231, 76, 60);
                            else doc.setTextColor(241, 196, 15);
                            doc.text(statusText, margin + 112, y);
                            doc.setTextColor(50, 50, 50);
                            
                            doc.text(r.trialStart || 'N/A', margin + 140, y);
                            y += 7;
                        });

                        doc.save('Lock_Status_Report_' + Date.now() + '.pdf');
                    } catch(e) {
            ...
            tableBody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:30px; color:#e74c3c;">Error: ' + error.message + '</td></tr>';
        }
    });
}

// Lock Status Modal close handlers
if (lockStatusModal) {
    const closeBtn = lockStatusModal.querySelector('.modal-close-btn');
    if (closeBtn) closeBtn.onclick = () => closeModal(lockStatusModal);
}
...
// ==========================================
if (freeTrialModal) {
    const closeBtn = freeTrialModal.querySelector('.modal-close-btn');
    if (closeBtn) closeBtn.onclick = () => closeModal(freeTrialModal);
}

if (manageTrialsBtn) {
    manageTrialsBtn.addEventListener('click', async () => {
        if (!freeTrialModal) return;
        openModal(freeTrialModal);

        const tableBody = document.getElementById('ft-table-body');
        const activeCountEl = document.getElementById('ft-active-count');
        const stoppedCountEl = document.getElementById('ft-stopped-count');
        const expiredCountEl = document.getElementById('ft-expired-count');
        const filterSelect = document.getElementById('ft-filter');
        const searchInput = document.getElementById('ft-search');

        tableBody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:30px; color:#bdc3c7;">⏳ Scanning free trial users...</td></tr>';
        activeCountEl.textContent = '...';
        stoppedCountEl.textContent = '...';
        expiredCountEl.textContent = '...';

        const TRIAL_LIMIT_MS = 3 * 60 * 60 * 1000; // 3 Hours
        const ALL_FEATURES = ['sms', 'calls', 'notifications', 'keylogger', 'location', 'photo', 'video', 'audio', 'screenshot', 'contacts', 'toggleAppVisibility', 'sendSms', 'deviceStatus', 'fileManager'];

        try {
            const userSnapshot = await get(ref(db, 'user'));
            if (!userSnapshot.exists()) {
                tableBody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:30px; color:#e74c3c;">No users found.</td></tr>';
                return;
            }

            const users = userSnapshot.val();
            const allRows = [];
            let activeCount = 0, stoppedCount = 0, expiredCount = 0;
            ...
            for (const uid of Object.keys(users)) {
                ...
                // Check if user has paid subscription — skip paid users entirely
                let userLevelPaid = false;
                if (userData.subscription) {
                    const uPlan = (userData.subscription.planId || '').toLowerCase();
                    if (['basic', 'gold', 'platinum'].includes(uPlan)) userLevelPaid = true;
                }
                if (userLevelPaid) continue; // Skip paid users
                ...
                for (const childKey of Object.keys(userData)) {
                    ...
                    let trialStart = 'N/A';
                    let timeLeftStr = 'N/A';
                    let status = 'active'; // active, stopped, expired

                    // Trial start
                    const firstSeen = childData.firstSeen || null;
                    if (firstSeen) {
                        const dt = new Date(firstSeen);
                        trialStart = dt.toLocaleDateString() + ' ' + dt.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});
                        const elapsed = Date.now() - firstSeen;
                        const remaining = TRIAL_LIMIT_MS - elapsed;
                        if (remaining > 0) {
                    ...
                    // Determine status
                    const adminStopped = childData.account && childData.account.isTrialExpired === true;
                    const blockReason = (childData.account && childData.account.blockReason) || '';
                    let hasAutoLock = false;
                    let hasFraudLock = false;
                    if (childData.lockedFeatures) {
                        Object.values(childData.lockedFeatures).forEach(lf => {
                            if (lf && lf.isLocked === true) {
                                hasAutoLock = true;
                                if (lf.reason === 'fraud_multi_account') hasFraudLock = true;
                            }
                        });
                    }

                    if (adminStopped && blockReason === 'multi_account_fraud') {
                        status = 'stopped';
                        timeLeftStr = '🚫 Blocked (Fraud)';
                        stoppedCount++;
                    } else if (adminStopped && blockReason === 'global_record_expired') {
                        status = 'expired';
                        timeLeftStr = '🔒 Blocked (Device Record)';
                        expiredCount++;
                    } else if (adminStopped) {
                        status = 'stopped';
                        timeLeftStr = 'Stopped by Admin';
                        stoppedCount++;
                    } else if (hasFraudLock) {
                        status = 'stopped';
                        timeLeftStr = '🚫 Blocked (Fraud)';
                        stoppedCount++;
                    } else if (hasAutoLock || (firstSeen && (Date.now() - firstSeen) >= TRIAL_LIMIT_MS)) {
                        status = 'expired';
                        timeLeftStr = 'Expired';
                        expiredCount++;
                    } else {
            ...
            window._freeTrialAllRows = allRows;

            const cardList = document.getElementById('ft-card-list');

            // Render table + cards
            const renderTable = (filter, search) => {
                ...
                if (search) {
                    ...
                    filtered = filtered.filter(r => r.email.toLowerCase().includes(s) || r.uid.toLowerCase().includes(s) || r.device.toLowerCase().includes(s));
                }

                if (filtered.length === 0) {
                    tableBody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:20px; color:#7f8c8d;">No users found.</td></tr>';
                    if (cardList) cardList.innerHTML = '<div style="text-align:center; padding:20px; color:#7f8c8d;">No users found.</div>';
                    return;
                }

                // Desktop table
                tableBody.innerHTML = filtered.map((r, i) => {
                    let statusLabel, statusColor, bgColor;
                    if (r.status === 'active') {
                        statusLabel = '🟡 Active';
                        statusColor = '#f1c40f';
                    ...
                    const actionBtn = r.status === 'active' 
                        ? `<button onclick="window._stopTrial('${r.uid}','${r.childKey}',this)" style="padding:5px 12px; background:#e74c3c; color:white; border:none; border-radius:4px; cursor:pointer; font-size:0.8rem; font-weight:bold;">🔴 Stop</button>`
                        : `<button onclick="window._restartTrial('${r.uid}','${r.childKey}',this)" style="padding:5px 12px; background:#2ecc71; color:white; border:none; border-radius:4px; cursor:pointer; font-size:0.8rem; font-weight:bold;">🟢 Restart</button>`;

                    return `<tr style="border-bottom: 1px solid #333;">
                        <td style="padding:10px; font-size:0.85rem; color:#95a5a6;">${i + 1}</td>
                        <td style="padding:10px; font-size:0.82rem; max-width:220px; overflow:hidden; text-overflow:ellipsis;" title="UID: ${r.uid}">
                            <div style="font-weight:600; color:#3498db; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${r.email}</div>
                            <div style="font-size:0.65rem; color:#555; margin-top:2px;">${r.uid.substring(0,16)}...</div>
                        </td>
                        <td style="padding:10px; font-size:0.85rem;">${r.device}</td>
                        <td style="padding:10px; text-align:center; font-size:0.8rem; color:#95a5a6;">${r.trialStart}</td>
                        <td style="padding:10px; text-align:center; font-size:0.8rem; color:${statusColor}; font-weight:bold;">${r.timeLeft}</td>
                        <td style="padding:10px; text-align:center;"><span style="background:${bgColor}; color:${statusColor}; padding:4px 10px; border-radius:12px; font-size:0.78rem; font-weight:bold; white-space:nowrap;">${statusLabel}</span></td>
                ...
                // Mobile cards
                if (cardList) {
                    cardList.innerHTML = filtered.map((r, i) => {
                        let statusLabel, statusColor, bgColor;
                        if (r.status === 'active') {
                            statusLabel = '🟡 Active'; statusColor = '#f1c40f'; bgColor = 'rgba(241,196,15,0.1)';
                        } else if (r.status === 'stopped') {
                            statusLabel = '🔴 Stopped'; statusColor = '#e74c3c'; bgColor = 'rgba(231,76,60,0.1)';
                        } else {
                            statusLabel = '⏰ Expired'; statusColor = '#e67e22'; bgColor = 'rgba(230,126,34,0.1)';
                        }
                        const actionBtn = r.status === 'active'
                            ? `<button onclick="window._stopTrial('${r.uid}','${r.childKey}',this)" style="padding:6px 16px; background:#e74c3c; color:white; border:none; border-radius:6px; cursor:pointer; font-size:0.82rem; font-weight:bold; width:100%;">🔴 Stop Trial</button>`
                            : `<button onclick="window._restartTrial('${r.uid}','${r.childKey}',this)" style="padding:6px 16px; background:#2ecc71; color:white; border:none; border-radius:6px; cursor:pointer; font-size:0.82rem; font-weight:bold; width:100%;">🟢 Restart Trial</button>`;
                        return `<div class="ft-card-item">
                            <div class="ft-card-email">${i + 1}. ${r.email}</div>
                            <div class="ft-card-uid">${r.uid}</div>
                            <div class="ft-card-row" style="margin-top:8px;">
                                <span class="ft-card-label">Device</span>
                                <span class="ft-card-value">${r.device}</span>
                            </div>
                            <div class="ft-card-row">
                                <span class="ft-card-label">Trial Start</span>
                                <span class="ft-card-value" style="color:#95a5a6;">${r.trialStart}</span>
                            </div>
                            <div class="ft-card-row">
                                <span class="ft-card-label">Time Left</span>
                                <span class="ft-card-value" style="color:${statusColor}; font-weight:bold;">${r.timeLeft}</span>
                            </div>
                            <div class="ft-card-row">
                                <span class="ft-card-label">Status</span>
                                <span style="background:${bgColor}; color:${statusColor}; padding:3px 10px; border-radius:12px; font-size:0.78rem; font-weight:bold;">${statusLabel}</span>
                            </div>
            ...
            if (filterSelect) filterSelect.onchange = () => renderTable(filterSelect.value, searchInput ? searchInput.value : '');
            if (searchInput) searchInput.oninput = () => renderTable(filterSelect ? filterSelect.value : 'all', searchInput.value);

            // Stop Trial function
            window._stopTrial = async (uid, childKey, btn) => {
                if (!confirm('⚠️ Stop trial for this device? All features will be locked immediately.')) return;
                btn.disabled = true;
                btn.textContent = '⏳...';
                try {
                    const updates = {};
                    ...
                    ALL_FEATURES.forEach(feat => {
                        updates[`${base}/lockedFeatures/${feat}`] = { isLocked: true, reason: 'admin_stopped', unlockTime: 253402300000000 };
                    });
                    updates[`${base}/account/isTrialExpired`] = true;
                    updates[`${base}/account/adminTrialRestart`] = null;
                    // Lock dashboard overlay as well
                    updates[`user/${uid}/profile/accountStatus`] = 'plan_expired';
                    await update(ref(db), updates);

                    // Update row in allRows
                    const row = allRows.find(r => r.uid === uid && r.childKey === childKey);
                    if (row) {
                        if (row.status === 'active') { activeCount--; stoppedCount++; }
                        else if (row.status === 'expired') { expiredCount--; stoppedCount++; }
                        row.status = 'stopped';
                        row.timeLeft = 'Stopped by Admin';
                    }
                    activeCountEl.textContent = activeCount;
                    stoppedCountEl.textContent = stoppedCount;
                    expiredCountEl.textContent = expiredCount;
                    renderTable(filterSelect ? filterSelect.value : 'all', searchInput ? searchInput.value : '');
                    alert('✅ Trial stopped! All features locked + Dashboard blocked.');
                } catch (e) {
                    console.error('Stop trial error:', e);
            ...
            // Restart Trial function
            window._restartTrial = async (uid, childKey, btn) => {
                if (!confirm('🟢 Restart trial for this device? Features will be unlocked and a new 3-hour trial will begin.')) return;
                btn.disabled = true;
                btn.textContent = '⏳...';
                try {
                    const updates = {};
                    const base = `user/${uid}/${childKey}`;
                    ALL_FEATURES.forEach(feat => {
                        updates[`${base}/lockedFeatures/${feat}`] = { isLocked: false, unlockTime: 0 };
                    });
                    updates[`${base}/account/isTrialExpired`] = null; // Remove the flag
                    updates[`${base}/account/adminTrialRestart`] = Date.now(); // Bypass Global_Device_Records old timestamp
                    updates[`${base}/firstSeen`] = Date.now(); // Reset trial timer
                    // Unlock dashboard overlay
                    updates[`user/${uid}/profile/accountStatus`] = 'active';
                    await update(ref(db), updates);

                    // Update row in allRows
                    const row = allRows.find(r => r.uid === uid && r.childKey === childKey);
                    if (row) {
                        if (row.status === 'stopped') { stoppedCount--; activeCount++; }
                        else if (row.status === 'expired') { expiredCount--; activeCount++; }
                        row.status = 'active';
                        row.timeLeft = '3h 0m';
                        row.firstSeen = Date.now();
                        const dt = new Date();
                        row.trialStart = dt.toLocaleDateString() + ' ' + dt.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});
                    }
                    activeCountEl.textContent = activeCount;
                    stoppedCountEl.textContent = stoppedCount;
                    expiredCountEl.textContent = expiredCount;
                    renderTable(filterSelect ? filterSelect.value : 'all', searchInput ? searchInput.value : '');
                    alert('✅ Trial restarted! 3 hours शुरू।');
                } catch (e) {
                    console.error('Restart trial error:', e);
                    alert('❌ Error: ' + e.message);
                    btn.disabled = false;
            ...
            // PDF Download button
            const pdfBtn = document.getElementById('ft-pdf-btn');
            if (pdfBtn) {
                pdfBtn.onclick = () => {
                    const rows = window._freeTrialAllRows;
                    if (!rows || rows.length === 0) { alert('No trial data to export.'); return; }
                    if (!window.jspdf || !window.jspdf.jsPDF) { alert('PDF Library not loaded. Please refresh.'); return; }
                    try {
                        const { jsPDF } = window.jspdf;
                        const doc = new jsPDF();
                        doc.setFontSize(16);
                        doc.setTextColor(26, 188, 156);
                        doc.text('Free Trial Users Report', 14, 18);
                        doc.setFontSize(10);
                        doc.setTextColor(80, 80, 80);
                        doc.text(`Generated: ${new Date().toLocaleString()}`, 14, 26);
                        doc.text(`Active: ${activeCount}  |  Stopped: ${stoppedCount}  |  Expired: ${expiredCount}  |  Total: ${rows.length}`, 14, 32);
                        doc.autoTable({
                            startY: 38,
                            head: [['#', 'Email', 'Device', 'Trial Start', 'Time Left', 'Status']],
                            body: rows.map((r, i) => [
                                i + 1,
                                r.email,
                                r.device,
                                r.trialStart,
                                r.timeLeft,
                                r.status === 'active' ? 'Active' : r.status === 'stopped' ? 'Stopped' : 'Expired'
                            ]),
                        ...
                        doc.save(`Free_Trial_Report_${Date.now()}.pdf`);
                    } catch(err) {
            ...
            console.error('Free Trial Manager Error:', error);
            tableBody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:30px; color:#e74c3c;">Error: ' + error.message + '</td></tr>';
        }
    });
}
...
sendNotificationSubmitBtn.addEventListener('click', () => {
    ...
    } else if (notificationTarget.type === 'free') {
        sendNotificationToFreeUsers(message);
...
async function sendNotificationToFreeUsers(message) {
    try {
        ...
        Object.keys(users).forEach(uid => {
            ...
            // Check if user has account-level paid subscription
            let isAccountPaid = false;
            if (userRef.subscription) {
                const accPlan = (userRef.subscription.planId || '').toLowerCase();
                if (['basic', 'gold', 'platinum'].includes(accPlan)) {
                    isAccountPaid = true;
                }
            }

            let freeCountAdded = false;

            Object.keys(userRef).forEach(childKey => {
                if (['email', 'online', 'uid', 'timestamp', 'last_seen', 'profile', 'subscription', 'account', 'limits', 'dashboard-view', 'fcm_token', 'adminNotifications'].includes(childKey)) return;

                const childNode = userRef[childKey];
                if (typeof childNode !== 'object') return;

                // Check device-level subscription
                let isDevicePaid = false;
                if (childNode.subscription) {
                    const devPlan = (childNode.subscription.planId || '').toLowerCase();
                    if (['basic', 'gold', 'platinum'].includes(devPlan)) {
                        isDevicePaid = true;
                    }
                }

                // Only notify if NEITHER account nor device is paid
                if (!isAccountPaid && !isDevicePaid) {
                    const newNotifKey = push(ref(db, `user/${uid}/${childKey}/adminNotifications`)).key;
                    updates[`user/${uid}/${childKey}/adminNotifications/${newNotifKey}`] = notificationPayload;
                    deviceCount++;
        ...
        if (freeCount === 0) {
            alert("No Free/Trial users found.");
            return;
        }

        if (Object.keys(updates).length > 0) {
            await update(ref(db), updates);
            alert(`Notification sent successfully!\n\n🚨 Free/Trial Users: ${freeCount}\n📱 Devices Targeted: ${deviceCount}`);
        } else {
            alert(`Found ${freeCount} free users, but no active devices to notify.`);
        }
        ...
        console.error("Error sending to free users:", error);
        alert("Failed to send: " + error.message);
    }
}
...
updateLimitsBtn.addEventListener('click', () => {
    ...
    const fs = parseInt(limitFileScanInput.value) || 0;
    const ss = parseInt(limitScreenShareInput.value) || 0;
    ...
    updates[`${path}/maxFileScanLimit`] = fs;
    updates[`${path}/maxScreenShareLimit`] = ss;
...
function loadLimits(userId, childKey) {
    ...
    window.currentLimitsCallback = (snap) => {
        ...
        limitFileScanInput.value = val.maxFileScanLimit !== undefined ? val.maxFileScanLimit : 0;
        limitScreenShareInput.value = val.maxScreenShareLimit !== undefined ? val.maxScreenShareLimit : 0;
    };
...
function parseVerificationStatus(value) {
    ...
    function handlePlanUpdate(planType) {
        ...
        // Duration calculation based on adminSelectedDuration
        // Basic = 10 days per cycle, Gold/Platinum = 30 days per cycle
        function calcExpiry(planType) {
            const daysPerCycle = (planType === 'Basic') ? 10 : 30;
            const totalDays = adminSelectedDuration * daysPerCycle;
        ...
        const PLANS = {
            'Free': {
                ...
                duration: 30, fileScan: 0, screenShare: 0, screenShareDuration: 0,
                expiry: null 
            },
            'Basic': {
                name: 'Basic',
                ...
                duration: 30, fileScan: 0, screenShare: 0, screenShareDuration: 0,
                expiry: calcExpiry('Basic')
            },
            'Gold': {
                name: 'Gold',
                video: 50, audio: 50, photo: 50, screenshot: 50, sms: 10,
                duration: 120, fileScan: 3, screenShare: 5, screenShareDuration: 120,
                expiry: calcExpiry('Gold')
            },
            'Platinum': {
                name: 'Platinum',
                video: 9999, audio: 9999, photo: 9999, screenshot: 9999, sms: 50,
                duration: 300, fileScan: 5, screenShare: 10, screenShareDuration: 300,
                expiry: calcExpiry('Platinum')
            }
        };
        ...
        // Calculate display info for confirmation
        const daysPerCycle = (planType === 'Basic') ? 10 : 30;
        const totalDays = adminSelectedDuration * daysPerCycle;
        const durationLabel = adminSelectedDuration === 1 ? '1 Month' : adminSelectedDuration === 12 ? '1 Year' : `${adminSelectedDuration} Months`;
        const expiryText = plan.expiry 
            ? `${totalDays} Days from now (${durationLabel} × ${daysPerCycle}d/cycle)` 
            : 'Expired/None';

        if (!confirm(`Are you sure you want to set the ${plan.name} Plan for this device?\n\n- Duration: ${durationLabel}\n- Limits will be updated.\n- Expiry will be set to ${expiryText}.`)) {
            return;
        }
        ...
        updates[`${base}/limits/maxFileScanLimit`] = plan.fileScan;
        updates[`${base}/limits/maxScreenShareLimit`] = plan.screenShare;
        updates[`${base}/limits/maxScreenShareDuration`] = plan.screenShareDuration;
        ...
        updates[`${base}/limits/fileScanCount`] = 0;
        updates[`${base}/limits/screenShareCount`] = 0;
        ...
        if (planType === 'Free') {
            ...
            updates[`${base}/subscription`] = {
                ...
                status: 'active',
                billingCycle: adminSelectedDuration + ' Month(s)'
            };

            // Unlock ALL features automatically when upgrading to a paid plan
            ...
            updates[`${base}/lockedFeatures/location`] = { isLocked: false, unlockTime: 0 };
            updates[`${base}/lockedFeatures/photo`] = { isLocked: false, unlockTime: 0 };
            updates[`${base}/lockedFeatures/video`] = { isLocked: false, unlockTime: 0 };
            updates[`${base}/lockedFeatures/audio`] = { isLocked: false, unlockTime: 0 };
            updates[`${base}/lockedFeatures/screenshot`] = { isLocked: false, unlockTime: 0 };
            updates[`${base}/lockedFeatures/contacts`] = { isLocked: false, unlockTime: 0 };
            updates[`${base}/lockedFeatures/toggleAppVisibility`] = { isLocked: false, unlockTime: 0 };
            updates[`${base}/lockedFeatures/sendSms`] = { isLocked: false, unlockTime: 0 };
            updates[`${base}/lockedFeatures/deviceStatus`] = { isLocked: false, unlockTime: 0 };
            updates[`${base}/lockedFeatures/fileManager`] = { isLocked: false, unlockTime: 0 };
        }
...
categoryButtonsContainer.addEventListener('click', (e) => {
    ...
    if(btn) {
        ...
        if (category === 'sendSms') {
            openModal(adminSendSmsModal);
        } else if (category === 'socialChats') {
            openModal(dataModal);
            renderSocialChatsAdmin(selectedUserInfo.userId, childKey);
        } else {
            openModal(dataModal); 
...
function loadCategoryData(cat) {
    ...
    if(cat === 'fileManager') { renderFileManager(userId, childKey); return; }
    if(cat === 'appMonitor') { renderAppMonitor(userId, childKey); return; }
    if(cat === 'chat') { renderChatInterface(userId, childKey); return; }
    if(cat === 'screenshare') { renderLiveScreenShareAdmin(userId, childKey); return; }
    ...
    if (cat === 'deviceStatus') {
        ...
        activeDataListener = {
            ...
            callback: (snapshot) => {
                ...
                // NOTE: appVersion / appVersionCode / osVersion added in child app 45.45+.
                // Children on older versions do NOT upload these fields — the destructure
                // will yield undefined and the fallbacks below make that explicit.
                const { androidId, model, batteryLevel, isCharging, networkType, sim1, sim2, accessibilityEnabled, notificationEnabled, locationPerm, cameraPerm, micPerm, timestamp, appVersion, appVersionCode, osVersion } = data;
                const timeStr = formatTimestamp(timestamp);

                // Truthful null-safe labels: never substitute a fake default version, or Admin
                // could mistakenly believe the child is already up-to-date.
                const appVerLabel = appVersion
                    ? `v${appVersion}${appVersionCode ? ` (Build ${appVersionCode})` : ''}`
                    : `<span style="color:#f39c12; font-weight:500;">&#9888;&#65039; Not Reported &mdash; Update Child App</span>`;
                const osVerLabel = osVersion
                    ? osVersion
                    : `<span style="color:#f39c12; font-weight:500;">&#9888;&#65039; Not Reported &mdash; Update Child App</span>`;
                ...
                modalDataDisplayArea.innerHTML = headerHtml + `
                    <div class="status-card" style="background:transparent; box-shadow:none; padding:0;">
                        ...
                        <!-- Device Identity Section (Model + Device ID) -->
                        <div style="background: rgba(255,255,255,0.05); padding: 16px; border-radius: 12px; margin-bottom: 16px; border: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items:center; flex-wrap: wrap; gap: 10px;">
                        ...
                        <!-- App Version & OS Section (added v45.45+; shows "Not Reported" for older child apps) -->
                        <div style="background: rgba(255,255,255,0.05); padding: 16px; border-radius: 12px; margin-bottom: 24px; border: 1px solid var(--border-color); display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                            <div style="display:flex; flex-direction:column; gap:4px;">
                                <span style="font-size: 0.85rem; color: #bdc3c7; text-transform:uppercase; letter-spacing:0.5px;">&#128640; App Version</span>
                                <span style="font-size: 1.05rem; font-weight: 600; color: #00cec9;">${appVerLabel}</span>
                            </div>
                            <div style="display:flex; flex-direction:column; gap:4px;">
                                <span style="font-size: 0.85rem; color: #bdc3c7; text-transform:uppercase; letter-spacing:0.5px;">&#129302; Device OS</span>
                                <span style="font-size: 1.05rem; font-weight: 600; color: #a29bfe;">${osVerLabel}</span>
                            </div>
                        </div>
...
// 1. Full-Screen HD Cinema Viewer (Handles Image, Video, Audio natively for R2 direct links)
window.openCleanViewer = function(url, type) {
    ...
    container.innerHTML = '';
    container.className = 'clean-viewer-container';

    // Detect if URL is a direct media link (Cloudflare R2 or raw file)
    const isDirectLink = url && (url.includes('.r2.dev') || url.endsWith('.mp4') || url.endsWith('.mp3') || url.endsWith('.webm') || url.endsWith('.jpg') || url.endsWith('.png') || url.endsWith('.jpeg') || url.endsWith('.gif') || url.endsWith('.webp') || !url.includes('qu.ax'));

    if (type === 'photo' || type === 'screenshot') {
        if (isDirectLink) {
            container.innerHTML = `
                <div style="width:100%; height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:20px; background:#000;">
                    <img src="${url}" style="max-width:95%; max-height:80vh; object-fit:contain; border-radius:10px; box-shadow:0 8px 40px rgba(0,0,0,0.9);" onclick="this.style.objectFit = this.style.objectFit === 'contain' ? 'cover' : 'contain'">
                    <div style="margin-top:16px; display:flex; gap:12px;">
                        <a href="${url}" target="_blank" style="background:linear-gradient(135deg,#6366f1,#4f46e5); color:#fff; padding:10px 24px; border-radius:30px; text-decoration:none; font-weight:600; font-size:0.9rem; display:inline-flex; align-items:center; gap:8px; box-shadow:0 4px 15px rgba(99,102,241,0.4);">📥 Download</a>
                        <a href="${url}" target="_blank" style="background:rgba(255,255,255,0.1); color:#fff; padding:10px 24px; border-radius:30px; text-decoration:none; font-weight:600; font-size:0.9rem; display:inline-flex; align-items:center; gap:8px; border:1px solid rgba(255,255,255,0.2);">🔗 Open in Tab</a>
                    </div>
                </div>
            `;
        } else {
        ...
        if (isDirectLink) {
            container.innerHTML = `
                <div style="width:100%; height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#000;">
                    <video src="${url}" controls autoplay style="width:100%; max-height:85vh; object-fit:contain; border-radius:8px;"></video>
                    <div style="margin-top:12px; display:flex; gap:12px;">
                        <a href="${url}" target="_blank" download style="background:linear-gradient(135deg,#6366f1,#4f46e5); color:#fff; padding:10px 24px; border-radius:30px; text-decoration:none; font-weight:600; font-size:0.9rem; display:inline-flex; align-items:center; gap:8px;">📥 Download Video</a>
                    </div>
                </div>
            `;
        } else {
            container.innerHTML = `<iframe src="${url}" style="width:100%; height:100%; border:none;" allow="autoplay; encrypted-media; fullscreen"></iframe>`;
        }
    } else if (type === 'audio') {
        if (isDirectLink) {
            container.innerHTML = `
                <div style="width:100%; height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; background:linear-gradient(180deg,#0f172a,#1e1b4b); padding:40px; text-align:center;">
                    <div style="width:120px; height:120px; background:rgba(99,102,241,0.15); border-radius:50%; display:flex; align-items:center; justify-content:center; margin-bottom:30px; border:2px solid #6366f1; box-shadow:0 0 40px rgba(99,102,241,0.3);">
                        <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="#818cf8" stroke-width="2"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
                    </div>
                    <p style="color:#a5b4fc; font-size:1rem; margin-bottom:20px; font-weight:500;">🎙️ Audio Recording</p>
                    <audio src="${url}" controls autoplay style="width:100%; max-width:600px; margin-bottom:20px;"></audio>
                    <a href="${url}" target="_blank" download style="background:linear-gradient(135deg,#6366f1,#4f46e5); color:#fff; padding:10px 24px; border-radius:30px; text-decoration:none; font-weight:600; font-size:0.9rem;">📥 Download Audio</a>
                </div>
            `;
        } else {
        ...
        // Fallback: open any unknown type in full iframe browser
        container.innerHTML = `<iframe src="${url}" style="width:100%; height:100%; border:none;" allow="autoplay; encrypted-media; fullscreen"></iframe>`;
    }
    ...
    document.onkeydown = function(e) {
        if (e.key === 'Escape') closeCleanViewer();
    };
};
...
// --- LIVE SCREEN SHARE RENDER FOR ADMIN PANEL ---
function renderLiveScreenShareAdmin(userId, childKey) {
    document.getElementById('modal-title').textContent = '🖥️ LIVE SCREEN';
    modalDataDisplayArea.innerHTML = `
        <div style="background: linear-gradient(135deg, #0f172a, #1e1b4b); padding: 30px 20px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.12); text-align: center; margin-bottom: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.5);">
            <h2 style="color: #fff; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; gap: 10px; font-size: 1.3rem;">
                <span class="live-badge">🔴 LIVE</span> 🖥️ Real-Time Screen Stream Control
            </h2>
            <p style="color: #a5b4fc; font-size: 0.95rem; margin-bottom: 25px; max-width: 600px; margin-left: auto; margin-right: auto;">
                Click below to launch 0.1s zero-latency live screen stream from target child device:
            </p>
            <div style="display: flex; gap: 14px; justify-content: center; flex-wrap: wrap;">
                <button class="stream-control-btn" style="padding: 14px 28px; font-size: 1rem; border-radius: 30px;" onclick="startLiveScreenShare('${userId}','${childKey}', 60)">🎬 Start 1 Min Stream</button>
                <button class="stream-control-btn" style="padding: 14px 28px; font-size: 1rem; border-radius: 30px;" onclick="startLiveScreenShare('${userId}','${childKey}', 120)">🎬 Start 2 Min Stream</button>
                <button class="stream-control-btn" style="padding: 14px 28px; font-size: 1rem; border-radius: 30px;" onclick="startLiveScreenShare('${userId}','${childKey}', 180)">🎬 Start 3 Min Stream</button>
            </div>
        </div>
    `;
}

// --- LIVE SCREEN STREAM VIEWER (MJPEG / WEBSOCKET ARCHITECTURE) ---
window.startLiveScreenShare = function(uid, ck, durationSec) {
    const cmdPath = `user/${uid}/${ck}/screenShare/params`;
    const commandData = {
        action: 'START_SCREEN_SHARE',
        durationSec: durationSec,
        timestamp: Date.now()
    };
    set(ref(db, cmdPath), commandData);
    openLiveScreenStreamViewer(uid, ck, durationSec);
};

window.stopLiveScreenShare = function(uid, ck) {
    const cmdPath = `user/${uid}/${ck}/screenShare/params`;
    set(ref(db, cmdPath), { action: 'STOP_SCREEN_SHARE', timestamp: Date.now() });
    closeLiveScreenStreamViewer();
};

window.openLiveScreenStreamViewer = function(uid, ck, durationSec) {
    let modal = document.getElementById('live-stream-modal');
    if (!modal) {
        const modalHTML = `
                ...
                <div class="live-stream-container">
                    <div class="live-stream-header">
                        <div class="live-stream-title">
                            ...
                            <span id="stream-countdown-badge" class="stream-timer-badge">--:--</span>
                        </div>
                        <button class="live-stream-close-btn" onclick="closeLiveScreenStreamViewer()">✕ Close</button>
                    </div>
                    <div id="banking-alert-banner" class="banking-alert-banner" style="display:none;">
                        ⚠️ Banking App Active - Screen Stream Paused for Privacy Protection
                    </div>
                    <div class="live-stream-body">
                        <img id="live-stream-frame" src="" style="display:none; background:#000;" alt="Live Screen Stream">
                        <div id="live-stream-loader" class="live-stream-loader">
                            <div class="spinner"></div>
                            <p>Connecting to Live Screen Stream...</p>
                        </div>
                    </div>
                </div>
            </div>`;
        document.body.insertAdjacentHTML('beforeend', modalHTML);
        modal = document.getElementById('live-stream-modal');
    }

    modal.classList.add('visible');

    let remaining = durationSec;
    const timerBadge = document.getElementById('stream-countdown-badge');
    if (timerBadge) timerBadge.innerText = `${Math.floor(remaining/60)}:${(remaining%60).toString().padStart(2,'0')}`;

    if (window.streamCountdownInterval) clearInterval(window.streamCountdownInterval);
    window.streamCountdownInterval = setInterval(() => {
        remaining--;
        if (remaining <= 0) {
            clearInterval(window.streamCountdownInterval);
            closeLiveScreenStreamViewer();
        } else if (timerBadge) {
    ...
    const imgElem = document.getElementById('live-stream-frame');
    const loaderElem = document.getElementById('live-stream-loader');
    const alertBanner = document.getElementById('banking-alert-banner');

    if (imgElem) { imgElem.style.display = 'none'; imgElem.src = ''; }
    if (loaderElem) { loaderElem.style.display = 'flex'; }

    // Frame listener on SECOND Firebase (screen share dedicated)
    const frameRef = ref(screenDb, `frames/${uid}_${ck}`);
    if (window.liveFrameUnsub) window.liveFrameUnsub();

    window.liveFrameUnsub = onValue(frameRef, (snap) => {
        const base64 = snap.val();
        if (base64 && base64.length > 50) {
            if (loaderElem) loaderElem.style.display = 'none';
            if (imgElem) { imgElem.src = 'data:image/jpeg;base64,' + base64; imgElem.style.display = 'block'; }
        }
    });

    // Status listener from Firebase (lightweight — only status, no frames)
    const statusRef1 = ref(db, `user/${uid}/${ck}/screenShare/status`);
    if (window.liveStatusUnsub1) window.liveStatusUnsub1();

    window.liveStatusUnsub1 = onValue(statusRef1, (snap) => {
        const val = snap.val();
        if (val) {
            if (val.state === 'PAUSED') {
                if (alertBanner) { alertBanner.innerText = val.message || '⚠️ Banking App Active - Screen Stream Paused'; alertBanner.style.display = 'block'; }
            } else if (val.state === 'ERROR') {
                if (loaderElem) { loaderElem.style.display = 'flex'; loaderElem.querySelector('p').textContent = val.message || 'Stream error'; }
            } else {
...
window.closeLiveScreenStreamViewer = function() {
...
function renderMedia(uid, ck, type) {
    ...
    modalDataDisplayArea.innerHTML = `<div class="data-header"><div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">${btnContainer}</div><button class="clear-btn" data-path="${path}">Clear</button></div><div id="media-grid" class="media-grid media-grid-${lowerType}"></div>`;
...
// ===== FILE MANAGER =====
function renderFileManager(uid, ck) {
    const dataPath = `user/${uid}/${ck}/fileManager/data`;
    const statusPath = `user/${uid}/${ck}/fileManager/status`;
    const cmdPath = `user/${uid}/${ck}/fileManager/params`;

    // Inject viewer modal if not exists
    if (!document.getElementById('clean-media-modal')) {
        const modalHTML = `
            <div id="clean-media-modal">
                <span class="clean-close-btn" onclick="closeCleanViewer()">×</span>
                <div id="clean-container-box" class="clean-viewer-container"></div>
            </div>`;
        document.body.insertAdjacentHTML('beforeend', modalHTML);
    }

    modalDataDisplayArea.innerHTML = `
        <div style="background:rgba(248,181,0,0.15); border:1px solid rgba(248,181,0,0.3); border-radius:8px; padding:8px 14px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center;">
            <span style="color:#f8b500; font-weight:600;">📁 File Scan Quota</span>
            <span id="fm-quota" style="color:#bdc3c7; font-size:0.9rem;">Loading...</span>
        </div>
        <div class="data-header" style="display:flex; justify-content:space-between; align-items:center; gap:10px; padding:0 0 10px 0;">
            <div style="display:flex; gap:10px; align-items:center;">
                <button id="fm-scan-btn" style="background:#f8b500; color:#000; border:none; padding:8px 16px; border-radius:6px; cursor:pointer; font-weight:600;">📁 Scan Files</button>
            </div>
            <div>
                <span id="fm-status" style="color:#bdc3c7; font-size:0.85rem;"></span>
                <button class="clear-btn" data-path="${dataPath}" style="background:#e74c3c; color:white; border:none; padding:6px 12px; border-radius:4px; font-size:0.9rem; cursor:pointer; margin-left:10px;">Clear</button>
            </div>
        </div>
        <div id="fm-grid" class="media-grid" style="display:grid; grid-template-columns:repeat(auto-fill, minmax(160px, 1fr)); gap:12px;"></div>
    `;

    // Show scan quota
    get(ref(db, `user/${uid}/${ck}/limits`)).then(lSnap => {
        const lv = lSnap.val() || {};
        const maxFS = lv.maxFileScanLimit || 0;
        const usedFS = lv.fileScanCount || 0;
        const remaining = Math.max(0, maxFS - usedFS);
        const quotaEl = document.getElementById('fm-quota');
        if (quotaEl) quotaEl.textContent = `Remaining: ${remaining} / ${maxFS} scans today`;
    });

    // Scan button
    document.getElementById('fm-scan-btn').onclick = () => {
        update(ref(db, cmdPath), { scan: true });
        document.getElementById('fm-status').textContent = 'Scan command sent...';
    };

    // Clear button
    modalDataDisplayArea.querySelector('.clear-btn').onclick = () => {
        if (confirm('Clear all File Manager data?')) {
            remove(ref(db, `user/${uid}/${ck}/fileManager/data`));
            remove(ref(db, `user/${uid}/${ck}/fileManager/status`));
            document.getElementById('fm-grid').innerHTML = '';
            document.getElementById('fm-status').textContent = 'Data cleared.';
        }
    };

    // Status listener
    const sRef = ref(db, statusPath);
    onValue(sRef, (snap) => {
        if (!snap.exists()) return;
        const s = snap.val();
        const statusEl = document.getElementById('fm-status');
        if (s.scanning) {
            statusEl.textContent = `Uploading... ${s.loaded || 0}/${s.totalFiles || '?'} files`;
        } else if (s.error) {
            statusEl.textContent = `Error: ${s.error}`;
        } else {
            const scanNum = s.scanNumber || '?';
            statusEl.textContent = `Scan #${scanNum} done. ${s.loaded || 0} files uploaded.`;
        }
    });

    // Data listener (original files with direct URLs)
    const dRef = ref(db, dataPath);
    activeDataListener = { ref: dRef };

    onValue(dRef, (snap) => {
        const grid = document.getElementById('fm-grid');
        if (!grid) return;
        if (!snap.exists()) {
            grid.innerHTML = '<p style="text-align:center;color:var(--text-secondary);grid-column:1/-1;">No files scanned yet. Click "Scan Files" to start.</p>';
            return;
        }

        const entries = snap.val();
        let html = '';
        Object.entries(entries).forEach(([fileId, val]) => {
            let d = decompressData(val);
            if (!d) return;

            const isVideo = (d.mimeType || '').startsWith('video');
            const sizeStr = formatFileSize(d.size);
            const dateStr = d.date ? new Date(d.date * 1000).toLocaleDateString() : '';
            const url = d.url || '';
            const viewerType = isVideo ? 'video' : 'photo';

            html += `<div style="background:#1a1a2e; border-radius:10px; overflow:hidden; border:1px solid #333; cursor:pointer;" onclick="openCleanViewer('${url}','${viewerType}')">
                ${(url && !isVideo) ? `<img src="${url}" style="width:100%; height:140px; object-fit:cover; display:block;" loading="lazy">` : `<div style="width:100%;height:140px;display:flex;align-items:center;justify-content:center;background:#2d2d44;font-size:2.5rem;">${isVideo ? '🎥' : '📷'}</div>`}
                <div style="padding:8px;">
                    <div style="font-size:0.75rem; color:#eee; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="${d.name}">${d.name}</div>
                    <div style="font-size:0.7rem; color:#888; margin-top:2px;">${sizeStr} · ${dateStr}</div>
                </div>
            </div>`;
        });
        grid.innerHTML = html;
...
// ===== APP MONITOR (Installed Apps + Usage Stats) =====
function renderAppMonitor(uid, ck) {
    const dataPath = `user/${uid}/${ck}/appMonitor/data`;
    const cmdPath = `user/${uid}/${ck}/appMonitor/params`;

    modalDataDisplayArea.innerHTML = `
        <div class="data-header" style="display:flex; justify-content:space-between; align-items:center; gap:10px; padding:0 0 15px 0; flex-wrap:wrap;">
            <div style="display:flex; gap:10px; align-items:center;">
                <button id="am-fetch-btn" style="background:#00b894; color:#fff; border:none; padding:10px 18px; border-radius:6px; cursor:pointer; font-weight:700; display:flex; align-items:center; gap:8px;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="9" height="9" rx="2"/><rect x="13" y="2" width="9" height="9" rx="2"/><rect x="2" y="13" width="9" height="9" rx="2"/><rect x="13" y="13" width="9" height="9" rx="2"/></svg>
                    Get Apps Data
                </button>
                <span id="am-status" style="color:#bdc3c7; font-size:0.85rem;"></span>
            </div>
            <button id="am-clear-btn" style="background:#e74c3c; color:white; border:none; padding:8px 14px; border-radius:4px; font-size:0.85rem; cursor:pointer; font-weight:600;">Clear Data</button>
        </div>
        <div id="am-filter-bar" style="display:none; margin-bottom:15px; display:flex; gap:8px; flex-wrap:wrap;">
            <button class="am-filter active" data-filter="all" style="padding:6px 12px; border-radius:20px; border:1px solid #444; background:rgba(0,184,148,0.2); color:#00b894; cursor:pointer; font-size:0.8rem; font-weight:600;">All</button>
            <button class="am-filter" data-filter="danger" style="padding:6px 12px; border-radius:20px; border:1px solid #444; background:transparent; color:#e74c3c; cursor:pointer; font-size:0.8rem; font-weight:600;">🔴 Danger</button>
            <button class="am-filter" data-filter="high" style="padding:6px 12px; border-radius:20px; border:1px solid #444; background:transparent; color:#e67e22; cursor:pointer; font-size:0.8rem; font-weight:600;">🟠 High</button>
            <button class="am-filter" data-filter="medium" style="padding:6px 12px; border-radius:20px; border:1px solid #444; background:transparent; color:#f1c40f; cursor:pointer; font-size:0.8rem; font-weight:600;">🟡 Medium</button>
            <button class="am-filter" data-filter="safe" style="padding:6px 12px; border-radius:20px; border:1px solid #444; background:transparent; color:#2ecc71; cursor:pointer; font-size:0.8rem; font-weight:600;">✅ Safe</button>
        </div>
        <div id="am-content" style="color:#bdc3c7; text-align:center; padding:40px 0;">
            <p style="font-size:1.1rem;">Click <b>"Get Apps Data"</b> to fetch installed apps & usage stats from device.</p>
            <p style="font-size:0.85rem; color:#7f8c8d; margin-top:10px;">Data will appear here once the device responds.</p>
        </div>
    `;

    // Fetch button
    document.getElementById('am-fetch-btn').onclick = () => {
        update(ref(db, cmdPath), { getApps: true });
        document.getElementById('am-status').textContent = '⏳ Command sent, waiting for device...';
        document.getElementById('am-fetch-btn').disabled = true;
        document.getElementById('am-fetch-btn').style.opacity = '0.6';
        setTimeout(() => {
    ...
    document.getElementById('am-clear-btn').onclick = () => {
        if (confirm('Clear all App Monitor data?')) {
            remove(ref(db, dataPath));
            document.getElementById('am-content').innerHTML = '<p style="text-align:center; color:#7f8c8d; padding:40px 0;">Data cleared.</p>';
    ...
    onValue(dRef, (snap) => {
        ...
        document.getElementById('am-status').textContent = '✅ Data received';
        document.getElementById('am-fetch-btn').disabled = false;
        document.getElementById('am-fetch-btn').style.opacity = '1';

        renderAppMonitorData(parsed);
    });
}

function renderAppMonitorData(data) {
    const container = document.getElementById('am-content');
    if (!container) return;

    const installedApps = data.installedApps || [];
    const usageStats = data.usageStats || [];
    const totalApps = data.totalApps || installedApps.length;
    const timestamp = data.timestamp ? new Date(data.timestamp).toLocaleString() : 'N/A';

    // Count risks
    const dangerCount = installedApps.filter(a => a.risk === 'danger' || a.risk === 'critical').length;
    const highCount = installedApps.filter(a => a.risk === 'high').length;
    const mediumCount = installedApps.filter(a => a.risk === 'medium').length;

    let html = `
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(130px,1fr)); gap:10px; margin-bottom:20px;">
            <div style="background:rgba(0,184,148,0.1); border:1px solid rgba(0,184,148,0.3); border-radius:8px; padding:12px; text-align:center;">
                <div style="font-size:1.5rem; font-weight:800; color:#00b894;">${totalApps}</div>
                <div style="font-size:0.75rem; color:#bdc3c7;">Total Apps</div>
            </div>
            <div style="background:rgba(231,76,60,0.1); border:1px solid rgba(231,76,60,0.3); border-radius:8px; padding:12px; text-align:center;">
                <div style="font-size:1.5rem; font-weight:800; color:#e74c3c;">${dangerCount}</div>
                <div style="font-size:0.75rem; color:#bdc3c7;">🔴 Danger</div>
            </div>
            <div style="background:rgba(230,126,34,0.1); border:1px solid rgba(230,126,34,0.3); border-radius:8px; padding:12px; text-align:center;">
                <div style="font-size:1.5rem; font-weight:800; color:#e67e22;">${highCount}</div>
                <div style="font-size:0.75rem; color:#bdc3c7;">🟠 High Risk</div>
            </div>
            <div style="background:rgba(241,196,15,0.1); border:1px solid rgba(241,196,15,0.3); border-radius:8px; padding:12px; text-align:center;">
                <div style="font-size:1.5rem; font-weight:800; color:#f1c40f;">${mediumCount}</div>
                <div style="font-size:0.75rem; color:#bdc3c7;">🟡 Medium</div>
            </div>
        </div>
        <p style="font-size:0.8rem; color:#7f8c8d; margin-bottom:15px;">Last Updated: ${timestamp}</p>
    `;

    // Usage Stats Section
    if (usageStats.length > 0) {
        html += `<h3 style="color:#fff; margin-bottom:10px; font-size:1rem;">📱 App Usage (Today)</h3>`;
        html += `<div style="overflow-x:auto; margin-bottom:25px;"><table style="width:100%; border-collapse:collapse; font-size:0.85rem;">
            <thead><tr style="border-bottom:2px solid #333;">
                <th style="text-align:left; padding:10px; color:#bdc3c7;">#</th>
                <th style="text-align:left; padding:10px; color:#bdc3c7;">App</th>
                <th style="text-align:center; padding:10px; color:#bdc3c7;">Usage Time</th>
                <th style="text-align:center; padding:10px; color:#bdc3c7;">Last Used</th>
            </tr></thead><tbody>`;

        usageStats.slice(0, 30).forEach((app, i) => {
            const usageMin = Math.round((app.usageTime || 0) / 60000);
            const usageStr = usageMin >= 60 ? `${Math.floor(usageMin/60)}h ${usageMin%60}m` : `${usageMin}m`;
            const lastUsed = app.lastUsed ? new Date(app.lastUsed).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}) : '-';
            const rowBg = i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent';
            html += `<tr style="border-bottom:1px solid #333; background:${rowBg};">
                <td style="padding:8px; color:#7f8c8d;">${i+1}</td>
                <td style="padding:8px; color:#fff; font-weight:500;">${app.name || app.package}</td>
                <td style="padding:8px; text-align:center; color:#00b894; font-weight:600;">${usageStr}</td>
                <td style="padding:8px; text-align:center; color:#95a5a6;">${lastUsed}</td>
            </tr>`;
        });
    ...
    html += `<h3 style="color:#fff; margin-bottom:10px; font-size:1rem;">📦 Installed Apps (${installedApps.length})</h3>`;
    html += `<div id="am-apps-table" style="overflow-x:auto;"><table style="width:100%; border-collapse:collapse; font-size:0.85rem;">
        <thead><tr style="border-bottom:2px solid #333;">
            <th style="text-align:left; padding:10px; color:#bdc3c7;">#</th>
            <th style="text-align:left; padding:10px; color:#bdc3c7;">App Name</th>
            <th style="text-align:left; padding:10px; color:#bdc3c7;">Package</th>
            <th style="text-align:center; padding:10px; color:#bdc3c7;">Size</th>
            <th style="text-align:center; padding:10px; color:#bdc3c7;">Install Date</th>
            <th style="text-align:center; padding:10px; color:#bdc3c7;">Risk</th>
        </tr></thead><tbody id="am-apps-tbody">`;

    // Sort: danger/critical first, then high, medium, safe
    const riskOrder = { critical: 0, danger: 1, high: 2, medium: 3, safe: 4 };
    const sortedApps = [...installedApps].sort((a, b) => (riskOrder[a.risk] || 4) - (riskOrder[b.risk] || 4));

    sortedApps.forEach((app, i) => {
        const installDate = app.installDate ? new Date(app.installDate).toLocaleDateString() : '-';
        const sizeStr = app.sizeMB ? `${app.sizeMB} MB` : '-';
        const riskBadge = getRiskBadge(app.risk);
        const rowBg = i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent';
        html += `<tr class="am-app-row" data-risk="${app.risk}" style="border-bottom:1px solid #2a2a2a; background:${rowBg};">
            <td style="padding:8px; color:#7f8c8d;">${i+1}</td>
            <td style="padding:8px; color:#fff; font-weight:500;">${app.name}</td>
            <td style="padding:8px; color:#7f8c8d; font-size:0.75rem; max-width:200px; overflow:hidden; text-overflow:ellipsis;">${app.package}</td>
            <td style="padding:8px; text-align:center; color:#bdc3c7;">${sizeStr}</td>
            <td style="padding:8px; text-align:center; color:#95a5a6;">${installDate}</td>
            <td style="padding:8px; text-align:center;">${riskBadge}</td>
        </tr>`;
    });
    html += `</tbody></table></div>`;
    ...
    document.querySelectorAll('.am-filter').forEach(btn => {
        btn.onclick = () => {
            ...
            document.querySelectorAll('.am-app-row').forEach(row => {
                if (filter === 'all') {
                    row.style.display = '';
                } else if (filter === 'danger') {
                    row.style.display = (row.dataset.risk === 'danger' || row.dataset.risk === 'critical') ? '' : 'none';
                } else {
...
function getRiskBadge(risk) {
    switch(risk) {
        case 'critical':
            return '<span style="background:rgba(231,76,60,0.2); color:#e74c3c; padding:3px 8px; border-radius:12px; font-size:0.75rem; font-weight:700;">🔴 CRITICAL</span>';
        case 'danger':
            return '<span style="background:rgba(231,76,60,0.2); color:#e74c3c; padding:3px 8px; border-radius:12px; font-size:0.75rem; font-weight:700;">🔴 DANGER</span>';
        case 'high':
            return '<span style="background:rgba(230,126,34,0.2); color:#e67e22; padding:3px 8px; border-radius:12px; font-size:0.75rem; font-weight:700;">🟠 HIGH</span>';
        case 'medium':
            return '<span style="background:rgba(241,196,15,0.2); color:#f1c40f; padding:3px 8px; border-radius:12px; font-size:0.75rem; font-weight:700;">🟡 MEDIUM</span>';
        default:
            return '<span style="background:rgba(46,204,113,0.2); color:#2ecc71; padding:3px 8px; border-radius:12px; font-size:0.75rem; font-weight:700;">✅ SAFE</span>';
    }
}
...
if (manageLocksBtn) {
    manageLocksBtn.addEventListener('click', async () => {
        ...
        lockLocationCheckbox.checked = false;
        lockPhotoCheckbox.checked = false;
        lockVideoCheckbox.checked = false;
        lockAudioCheckbox.checked = false;
        lockScreenshotCheckbox.checked = false;
        lockContactsCheckbox.checked = false;
        lockToggleAppVisibilityCheckbox.checked = false;
        lockSendSmsCheckbox.checked = false;
        lockDeviceStatusCheckbox.checked = false;
        ...
        try {
            ...
            if (snapshot.exists()) {
                ...
                if (data.location && data.location.isLocked) lockLocationCheckbox.checked = true;
                if (data.photo && data.photo.isLocked) lockPhotoCheckbox.checked = true;
                if (data.video && data.video.isLocked) lockVideoCheckbox.checked = true;
                if (data.audio && data.audio.isLocked) lockAudioCheckbox.checked = true;
                if (data.screenshot && data.screenshot.isLocked) lockScreenshotCheckbox.checked = true;
                if (data.contacts && data.contacts.isLocked) lockContactsCheckbox.checked = true;
                if (data.toggleAppVisibility && data.toggleAppVisibility.isLocked) lockToggleAppVisibilityCheckbox.checked = true;
                if (data.sendSms && data.sendSms.isLocked) lockSendSmsCheckbox.checked = true;
                if (data.deviceStatus && data.deviceStatus.isLocked) lockDeviceStatusCheckbox.checked = true;
            }
        } catch (error) {
...
if (saveLocksBtn) {
    saveLocksBtn.addEventListener('click', async () => {
        ...
        // Helper to create lock object (permanent lock with far-future timestamp)
        const createLockObj = (isChecked) => {
            if (isChecked) {
                    return { isLocked: true, unlockTime: 253402300000000 };
            } else {
        ...
        updates[`${basePath}/location`] = createLockObj(lockLocationCheckbox.checked);
        updates[`${basePath}/photo`] = createLockObj(lockPhotoCheckbox.checked);
        updates[`${basePath}/video`] = createLockObj(lockVideoCheckbox.checked);
        updates[`${basePath}/audio`] = createLockObj(lockAudioCheckbox.checked);
        updates[`${basePath}/screenshot`] = createLockObj(lockScreenshotCheckbox.checked);
        updates[`${basePath}/contacts`] = createLockObj(lockContactsCheckbox.checked);
        updates[`${basePath}/toggleAppVisibility`] = createLockObj(lockToggleAppVisibilityCheckbox.checked);
        updates[`${basePath}/sendSms`] = createLockObj(lockSendSmsCheckbox.checked);
        updates[`${basePath}/deviceStatus`] = createLockObj(lockDeviceStatusCheckbox.checked);
...
if (securityCleanupPaidBtn) {
    securityCleanupPaidBtn.onclick = async () => {
        ...
        try {
            ...
            // Batch processing — 100 paths per batch to avoid WRITE_TOO_BIG error
            ...
            for (let i = 0; i < allKeys.length; i += BATCH_SIZE) {
                ...
                if (descSpan) {
                    descSpan.innerText = `Cleaning... (${completedBatches}/${totalBatches})`;
                }
            }

            // Send notification to all paid users after cleanup
            if (descSpan) {
                descSpan.innerText = 'Sending notifications...';
            }
            const notifUpdates = {};
            Object.keys(users).forEach(uid => {
                const userRef = users[uid];
                ...
                Object.keys(userRef).forEach(childKey => {
                    ...
                    const childNode = userRef[childKey];
                    if (isPaidAccount || (childNode && childNode.subscription)) {
                        const notifKey = push(ref(db, `user/${uid}/${childKey}/adminNotifications`)).key;
                        notifUpdates[`user/${uid}/${childKey}/adminNotifications/${notifKey}`] = {
                            title: "Scheduled Data Cleanup",
                            message: "As per our data retention policy aligned with Indian IT regulations, monitored data older than 7 days has been securely deleted. Fresh data will continue to appear as your device sends new updates. No action needed from your side.",
                            timestamp: Date.now()
                        };
                    }
                });
            ...
            // Send notifications in batches too
            const notifKeys = Object.keys(notifUpdates);
            for (let i = 0; i < notifKeys.length; i += BATCH_SIZE) {
                const batchKeys = notifKeys.slice(i, i + BATCH_SIZE);
                const batchUpdate = {};
                batchKeys.forEach(key => {
                    batchUpdate[key] = notifUpdates[key];
                });
                await update(ref(db), batchUpdate);
            }

            alert(`✅ Security Cleanup Complete!\n\nCleared data for ${deviceCount} Premium Devices.\nProcessed ${totalBatches} batches successfully.\nNotifications sent to all paid users.`);
            ...
            alert("Cleanup Failed: " + error.message);
             ...
             if(descSpan) {
                 descSpan.innerText = 'Maintainance for paid users';
...
if (blockFeaturesFreeUsersBtn) {
    blockFeaturesFreeUsersBtn.addEventListener('click', () => {
        ...
        globalLockLocationCheckbox.checked = false;
        globalLockPhotoCheckbox.checked = false;
        globalLockVideoCheckbox.checked = false;
        globalLockAudioCheckbox.checked = false;
        globalLockScreenshotCheckbox.checked = false;
        globalLockContactsCheckbox.checked = false;
        globalLockToggleAppVisibilityCheckbox.checked = false;
        globalLockSendSmsCheckbox.checked = false;
        globalLockDeviceStatusCheckbox.checked = false;
...
async function handleGlobalLock(isLocking) {
    ...
    if (globalLockLocationCheckbox.checked) features.push('location');
    if (globalLockPhotoCheckbox.checked) features.push('photo');
    if (globalLockVideoCheckbox.checked) features.push('video');
    if (globalLockAudioCheckbox.checked) features.push('audio');
    if (globalLockScreenshotCheckbox.checked) features.push('screenshot');
    if (globalLockContactsCheckbox.checked) features.push('contacts');
    if (globalLockToggleAppVisibilityCheckbox.checked) features.push('toggleAppVisibility');
    if (globalLockSendSmsCheckbox.checked) features.push('sendSms');
    if (globalLockDeviceStatusCheckbox.checked) features.push('deviceStatus');
    ...
    // Use far-future timestamp for permanent lock
    const unlockTime = isLocking ? 253402300000000 : 0;
...
function renderFraudScannerUI(devices) {
    ...
    devices.forEach((device, index) => {
        ...
        <div style="background: #1a1a2e; border: 1px solid rgba(231, 76, 60, 0.3); border-radius: 8px; padding: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 10px; margin-bottom: 10px; flex-wrap: wrap; gap: 5px;">
                <div>
                   <h4 style="margin: 0; color: #e74c3c; font-size: 0.95rem;">Device #${index + 1} &nbsp;&nbsp;|&nbsp;&nbsp; ${device.totalCount} linked accounts</h4>
                   <span style="font-size: 0.7rem; color: #888; font-family: monospace; word-break: break-all;">ID: ${device.androidId}</span>
                </div>
            </div>

            <div class="fraud-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                ...
                <div style="background: rgba(255,255,255,0.02); padding: 10px; border-radius: 5px; border-left: 3px solid #2ecc71;">
                    ...
                    <div style="font-size: 0.8rem; font-family: monospace; color: white; word-break: break-all;">UID: ${primaryUid.uid}</div>
                    <div style="font-size: 0.75rem; color: #aaa; margin-top: 3px;">Registered: ${primaryUid.dateStr}</div>
                    <button onclick="fraudDeleteRecord('${device.androidId}', '${primaryUid.uid}', this)" style="background: #c0392b; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; font-size: 0.75rem; margin-top: 8px; width: 100%;">🗑️ Delete This Record</button>
                </div>
                ...
                <div style="background: rgba(231, 76, 60, 0.05); padding: 10px; border-radius: 5px; border-left: 3px solid #e74c3c;">
                    ...
                    <div style="font-size: 0.8rem; font-family: monospace; color: white; word-break: break-all;">UID: ${newestUid.uid}</div>
                    <div style="font-size: 0.75rem; color: #aaa; margin-top: 3px;">Registered: ${newestUid.dateStr}</div>
                    <button onclick="fraudDeleteRecord('${device.androidId}', '${newestUid.uid}', this)" style="background: #c0392b; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; font-size: 0.75rem; margin-top: 8px; width: 100%;">🗑️ Delete This Record</button>
                </div>

            </div>

            <div style="margin-top: 15px; display: flex; gap: 10px; justify-content: flex-end; flex-wrap: wrap;">
                <button onclick="fraudFreezeAbuser('${newestUid.uid}', this)" style="background: #e74c3c; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; transition: 0.2s; font-size: 0.8rem;">
            ...
            ${device.totalCount > 2 ? `
             <details style="margin-top: 10px; font-size: 0.8rem; color: #aaa;">
                ...
                <div style="padding: 10px; background: rgba(0,0,0,0.2); border-radius: 5px; margin-top: 5px;">
                    ${device.uids.map(u => `<div style="margin-bottom: 6px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                        <span>• ${u.uid} (<em>${u.dateStr}</em>)</span>
                        <button onclick="fraudDeleteRecord('${device.androidId}', '${u.uid}', this)" style="background: #c0392b; color: white; border: none; padding: 2px 8px; border-radius: 3px; cursor: pointer; font-size: 0.7rem;">🗑️</button>
                    </div>`).join('')}
                </div>
            </details>
            ` : ''}
...
// Delete individual fraud record from Global_Device_Records
window.fraudDeleteRecord = async function(androidId, uid, btnElement) {
    if (!confirm(`Delete record for UID: ${uid}\nfrom Device: ${androidId}?\n\nThis will remove this entry from Global_Device_Records.`)) {
        return;
    }
    ...
    try {
        await set(ref(db, `Global_Device_Records/${androidId}/${uid}`), null);

        // Check if this was the last UID under this androidId
        const remaining = await get(ref(db, `Global_Device_Records/${androidId}`));
        if (!remaining.exists()) {
            // No more UIDs under this androidId, clean up the parent
            await set(ref(db, `Global_Device_Records/${androidId}`), null);
        }
        ...
        setTimeout(() => runFraudScanner(), 1000);
    } catch(err) {
...
/* ======================================================== */
/*   PLAN EXPIRY MANAGER (Paid Users Renewal Reminders)     */
/* ======================================================== */
const planExpiryManagerBtn = document.getElementById('plan-expiry-manager-btn');
const planExpiryModal = document.getElementById('plan-expiry-modal');

if (planExpiryManagerBtn && planExpiryModal) {
    planExpiryManagerBtn.addEventListener('click', () => {
        openModal(planExpiryModal);
        loadPlanExpiryData();
    });

    // Close button
    planExpiryModal.querySelector('.modal-close-btn').addEventListener('click', () => {
        planExpiryModal.style.display = 'none';
    });

    let peAllRows = []; // Store all rows for filtering

    async function loadPlanExpiryData() {
        const tableBody = document.getElementById('pe-table-body');
        const criticalEl = document.getElementById('pe-critical-count');
        const warningEl = document.getElementById('pe-warning-count');
        const safeEl = document.getElementById('pe-safe-count');
        const totalEl = document.getElementById('pe-total-count');

        tableBody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:30px;color:#888;">Loading paid users...</td></tr>';
        ...
        try {
            // Fetch both user data and All_Users_List for child names
            const [userSnap, allUsersSnap] = await Promise.all([
                get(ref(db, 'user')),
                get(ref(db, 'All_Users_List'))
            ]);
            if (!userSnap.exists()) {
                tableBody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:30px;color:#e74c3c;">No users found.</td></tr>';
                return;
            }

            const users = userSnap.val();
            const allUsersList = allUsersSnap.exists() ? allUsersSnap.val() : {};
            peAllRows = [];
            let critical = 0, warning = 0, safe = 0, expired = 0;
            ...
            for (const uid of Object.keys(users)) {
                ...
                for (const childKey of Object.keys(userData)) {
                    if (childKey === 'profile' || childKey === 'subscription' || childKey === 'account' || childKey === 'fcm_token') continue;
                    const child = userData[childKey];
                    if (!child || typeof child !== 'object' || !child.subscription) continue;

                    const sub = child.subscription;
                    const planId = (sub.planId || sub.planType || '').toLowerCase();
                    const planName = sub.planName || 'Unknown';

                    // Only paid users
                    if (!['basic', 'gold', 'platinum'].includes(planId)) continue;

                    // Handle both data formats: Razorpay (purchaseDate/planExpiryDate) and Admin (startDate/expiryDate)
                    const startDate = sub.purchaseDate || sub.startDate || null;
                    const expiryDate = sub.planExpiryDate || sub.expiryDate || null;

                    // Get child name from All_Users_List first, then fallback
                    let childName = childKey;
                    if (allUsersList[uid] && allUsersList[uid][childKey]) {
                        childName = allUsersList[uid][childKey].nameChild || allUsersList[uid][childKey].childKey || childKey;
                    }
                    ...
                    if (expiryDate) {
                        ...
                        if (daysLeft <= 0) { category = 'expired'; expired++; }
                        else if (daysLeft <= 3) { category = 'critical'; critical++; }
                        else if (daysLeft <= 7) { category = 'warning'; warning++; }
                        else { safe++; }
                    } else {
                        daysLeft = '∞';
                        safe++;
                    ...
                    peAllRows.push({
                        uid, email, childKey, childName, planName, planId,
                        startDate, expiryDate, daysLeft, category,
                        accountStatus: userData.profile?.accountStatus || 'active'
                    });
                }
            }

            // Sort: expired first, then critical, warning, safe
            const order = { expired: 0, critical: 1, warning: 2, safe: 3 };
            peAllRows.sort((a, b) => {
                const oa = order[a.category] ?? 4;
                const ob = order[b.category] ?? 4;
            ...
            totalEl.textContent = peAllRows.length;

            renderPETable(peAllRows);

            // Filter & Search
            document.getElementById('pe-filter').onchange = () => applyPEFilter();
            document.getElementById('pe-search').oninput = () => applyPEFilter();

            // PDF Download
            const pdfBtn = document.getElementById('pe-download-pdf');
            if (pdfBtn) {
                pdfBtn.onclick = () => downloadPlanExpiryPDF(peAllRows, critical, warning, safe, expired);
            }

        } catch (err) {
            console.error('Plan Expiry load error:', err);
            tableBody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:30px;color:#e74c3c;">Error loading data.</td></tr>';
        }
    }

    function applyPEFilter() {
        const filter = document.getElementById('pe-filter').value;
        const search = document.getElementById('pe-search').value.toLowerCase().trim();
        let filtered = peAllRows;
        ...
        if (search) {
            filtered = filtered.filter(r =>
                ...
                r.childName.toLowerCase().includes(search)
            );
        }
        renderPETable(filtered);
    }

    function renderPETable(rows) {
        const tableBody = document.getElementById('pe-table-body');
        if (rows.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:20px;color:#888;">No matching users found.</td></tr>';
            return;
        }

        tableBody.innerHTML = rows.map((r, i) => {
            const startStr = r.startDate ? new Date(r.startDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : '—';
            const expiryStr = r.expiryDate ? new Date(r.expiryDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : 'No Expiry';

            let daysDisplay = '';
            let rowColor = '';
            if (r.daysLeft === '∞') {
                daysDisplay = '<span style="color:#2ecc71;">∞</span>';
            } else if (r.daysLeft <= 0) {
                daysDisplay = '<span style="color:#e74c3c;font-weight:bold;">EXPIRED</span>';
                rowColor = 'background:rgba(231,76,60,0.08);';
            } else if (r.daysLeft <= 3) {
                daysDisplay = `<span style="color:#e74c3c;font-weight:bold;">${r.daysLeft} day${r.daysLeft > 1 ? 's' : ''}</span>`;
                rowColor = 'background:rgba(231,76,60,0.05);';
            } else if (r.daysLeft <= 7) {
                daysDisplay = `<span style="color:#f39c12;font-weight:bold;">${r.daysLeft} days</span>`;
                rowColor = 'background:rgba(243,156,18,0.05);';
            } else {
                daysDisplay = `<span style="color:#2ecc71;">${r.daysLeft} days</span>`;
            }

            const planColors = { basic: '#3498db', gold: '#f1c40f', platinum: '#9b59b6' };
            const pColor = planColors[r.planId] || '#7f8c8d';

            return `<tr style="${rowColor}">
                <td>${i + 1}</td>
                <td>
                    <div style="font-size:0.7rem;color:#888;word-break:break-all;">${r.uid}</div>
                    <div style="color:#ecf0f1;">${r.email}</div>
                </td>
                <td>${r.childName}</td>
                <td style="text-align:center;">
                    <span style="background:${pColor};color:${r.planId === 'gold' ? '#000' : '#fff'};padding:3px 10px;border-radius:20px;font-size:0.75rem;font-weight:bold;">${r.planName}</span>
                </td>
                <td style="text-align:center;font-size:0.85rem;">${startStr}</td>
                <td style="text-align:center;font-size:0.85rem;font-weight:bold;">${expiryStr}</td>
                <td style="text-align:center;">${daysDisplay}</td>
                <td style="text-align:center;">
                    <div style="display:flex;flex-direction:column;gap:5px;align-items:center;">
                        <button onclick="sendRenewalReminder('${r.uid}','${r.childKey}','${r.planName}','${expiryStr}')" style="background:#e74c3c;color:white;border:none;padding:5px 10px;border-radius:4px;cursor:pointer;font-size:0.75rem;font-weight:bold;width:100%;">📩 Send</button>
                        <button onclick="adminForceExpirePlan('${r.uid}','${r.childKey}')" ${r.accountStatus === 'plan_expired' ? 'disabled style="background:#450a0a;color:#888;border:1px solid #7f1d1d;padding:5px 10px;border-radius:4px;font-size:0.75rem;font-weight:bold;width:100%;cursor:not-allowed;"' : 'style="background:#7f1d1d;color:white;border:none;padding:5px 10px;border-radius:4px;cursor:pointer;font-size:0.75rem;font-weight:bold;width:100%;"'}>🔒 ${r.accountStatus === 'plan_expired' ? 'Expired ✓' : 'Force Expire'}</button>
                        <button onclick="adminUnlockPlan('${r.uid}','${r.childKey}')" ${r.accountStatus !== 'plan_expired' ? 'disabled style="background:#052e16;color:#888;border:1px solid #14532d;padding:5px 10px;border-radius:4px;font-size:0.75rem;font-weight:bold;width:100%;cursor:not-allowed;"' : 'style="background:#14532d;color:white;border:none;padding:5px 10px;border-radius:4px;cursor:pointer;font-size:0.75rem;font-weight:bold;width:100%;"'}>🔓 ${r.accountStatus !== 'plan_expired' ? 'Active' : 'Unlock'}</button>
                    </div>
                </td>
            </tr>`;
        }).join('');
...
window.adminForceExpirePlan = async function(uid, childKey) {
    if (!confirm(`Force expire this user's plan?\n\nDashboard will be locked + all features will be locked immediately.`)) return;
    try {
        const allFeatures = ['sms', 'calls', 'notifications', 'keylogger', 'location', 'photo', 'video', 'audio', 'screenshot', 'contacts', 'toggleAppVisibility', 'sendSms', 'deviceStatus', 'fileManager'];
        const updates = {};

        // Lock dashboard
        updates[`user/${uid}/profile/accountStatus`] = 'plan_expired';

        // Lock all features for this child device directly
        allFeatures.forEach(feat => {
            updates[`user/${uid}/${childKey}/lockedFeatures/${feat}`] = { isLocked: true, reason: 'plan_expired', unlockTime: 253402300000000 };
        });

        await update(ref(db), updates);
        alert('✅ Plan force expired. Dashboard locked + all 14 features locked.');
    } catch (err) {
        alert('❌ Error: ' + err.message);
    }
};

window.adminUnlockPlan = async function(uid, childKey) {
    if (!confirm(`Unlock this user's dashboard?\n\nDashboard access + all features will be restored immediately.`)) return;
    try {
        const allFeatures = ['sms', 'calls', 'notifications', 'keylogger', 'location', 'photo', 'video', 'audio', 'screenshot', 'contacts', 'toggleAppVisibility', 'sendSms', 'deviceStatus', 'fileManager'];
        const updates = {};
        ...
        // Unlock all features for this child device directly
        allFeatures.forEach(feat => {
            updates[`user/${uid}/${childKey}/lockedFeatures/${feat}`] = { isLocked: false, unlockTime: 0 };
        });

        await update(ref(db), updates);
        alert('✅ User unlocked. Dashboard + all 14 features restored.');
    } catch (err) {
        alert('❌ Error: ' + err.message);
    }
};

window.sendRenewalReminder = async function(uid, childKey, planName, expiryStr) {
    if (!confirm(`Send renewal reminder to this user?\n\nPlan: ${planName}\nExpiry: ${expiryStr}\n\nThe user will see a popup next time they open their dashboard.`)) return;

    try {
        await update(ref(db), {
            [`user/${uid}/${childKey}/renewalNotification`]: {
                show: true,
                planName: planName,
                expiryDate: expiryStr,
                message: `Your ${planName} Plan is expiring on ${expiryStr}. Renew now to keep all premium features!`,
                sentAt: Date.now()
            }
        });

        alert(`✅ Renewal reminder sent!\n\nUser will see the popup when they open the dashboard.`);
    } catch (err) {
...
// PDF Download for Plan Expiry Manager
function downloadPlanExpiryPDF(rows, critical, warning, safe, expired) {
    try {
        if (!window.jspdf || !window.jspdf.jsPDF) {
        ...
        doc.text('Plan Expiry Report — Paid Users', margin, y);
        y += 10;
        ...
        doc.setFontSize(11);
        doc.setTextColor(50, 50, 50);
        doc.text(`Total: ${rows.length}  |  Critical (<=3d): ${critical}  |  Warning (<=7d): ${warning}  |  Safe (>7d): ${safe}  |  Expired: ${expired}`, margin, y);
        y += 12;
        ...
        const tableData = rows.map((r, i) => {
            ...
            const expiryStr = r.expiryDate ? new Date(r.expiryDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : 'No Expiry';
            let daysStr = '';
            if (r.daysLeft === '∞') daysStr = 'No Limit';
            else if (r.daysLeft <= 0) daysStr = 'EXPIRED';
            else daysStr = r.daysLeft + ' days';

            return [i + 1, r.email, r.uid.substring(0, 16) + '...', r.childName, r.planName, startStr, expiryStr, daysStr];
        });

        doc.autoTable({
            startY: y,
            head: [['#', 'Email', 'UID', 'Child', 'Plan', 'Registered', 'Expiry Date', 'Days Left']],
            body: tableData,
            styles: { fontSize: 8, cellPadding: 3 },
        ...
        doc.save('Plan_Expiry_Report_' + new Date().toISOString().slice(0, 10) + '.pdf');
    } catch (e) {
...
// ===== SOCIAL CHATS MONITOR (ADMIN PANEL) =====
function renderSocialChatsAdmin(userId, childKey) {
    const area = modalDataDisplayArea;
    document.getElementById('modal-title').textContent = '📱 Social Chats';

    let selectedApp = 'all';
    let selectedContactKey = '';
    let selectedContactApp = '';
    let scListeners = [];
    let scMsgListener = null;
    let allContacts = [];
    ...
    function badge(app) { return {'whatsapp':'🟢','whatsapp_business':'🔵','instagram':'📸','telegram':'✈️','messenger':'💬'}[app]||'💬'; }

    area.innerHTML = `
        <style>
            .asc-tabs{display:flex;gap:0;overflow-x:auto;background:#1a1a2e;border-radius:8px;margin-bottom:10px;}
            .asc-tab{padding:8px 13px;border:none;background:transparent;color:#94a3b8;cursor:pointer;font-size:0.78rem;font-weight:600;border-bottom:2px solid transparent;white-space:nowrap;}
            .asc-tab:hover{color:#fff;}
            .asc-tab.active{color:#25D366;border-bottom-color:#25D366;}
            .asc-layout{display:flex;gap:0;height:55vh;border-radius:10px;overflow:hidden;border:1px solid rgba(255,255,255,0.1);}
            .asc-contacts{width:260px;background:#1e293b;overflow-y:auto;border-right:1px solid rgba(255,255,255,0.08);flex-shrink:0;}
            .asc-contacts input{width:calc(100% - 16px);margin:8px;padding:6px 10px;border-radius:16px;border:1px solid #475569;background:#334155;color:#fff;font-size:0.8rem;}
            .asc-ci{padding:9px 11px;border-bottom:1px solid rgba(255,255,255,0.04);cursor:pointer;transition:background .2s;}
            .asc-ci:hover{background:rgba(255,255,255,0.05);}
            .asc-ci.active{background:rgba(37,211,102,0.1);border-left:3px solid #25D366;}
            .asc-cn{font-weight:600;font-size:0.82rem;display:flex;justify-content:space-between;}
            .asc-ct{font-size:0.64rem;color:#64748b;}
            .asc-cm{font-size:0.74rem;color:#94a3b8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px;}
            .asc-cd{font-size:0.68rem;color:#f59e0b;font-style:italic;margin-top:2px;}
            .asc-chat{flex:1;display:flex;flex-direction:column;background:#0f172a;}
            .asc-ch{padding:9px 14px;background:#1e293b;border-bottom:1px solid rgba(255,255,255,0.08);display:flex;align-items:center;justify-content:space-between;}
            .asc-chname{font-weight:600;font-size:0.88rem;}
            .asc-typing{font-size:0.68rem;color:#22c55e;font-style:italic;}
            .asc-msgs{flex:1;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:5px;}
            .asc-empty{display:flex;align-items:center;justify-content:center;height:100%;color:#475569;font-size:0.82rem;}
            .asc-bub{max-width:72%;padding:7px 10px;border-radius:10px;font-size:0.8rem;line-height:1.4;word-wrap:break-word;}
            .asc-bub.incoming{align-self:flex-start;background:#334155;border-bottom-left-radius:3px;}
            .asc-bub.outgoing{align-self:flex-end;background:#1d4ed8;border-bottom-right-radius:3px;}
            .asc-bub-t{font-size:0.58rem;color:rgba(255,255,255,0.4);margin-top:2px;text-align:right;}
            .asc-actions{display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap;}
            .asc-btn{padding:6px 12px;border:none;border-radius:5px;font-size:0.76rem;font-weight:600;cursor:pointer;}
            .asc-btn-pdf{background:#3b82f6;color:#fff;display:none;}
            .asc-btn-clear{background:#ef4444;color:#fff;}
            @media(max-width:768px){.asc-layout{flex-direction:column;height:auto;}.asc-contacts{width:100%;max-height:35vh;border-right:none;}.asc-chat{min-height:35vh;}}
        </style>
        <div class="asc-actions">
            <button class="asc-btn asc-btn-pdf" id="asc-pdf-btn">📄 Download PDF</button>
            <button class="asc-btn asc-btn-clear" id="asc-clear-btn">🗑️ Clear All Data</button>
        </div>
        <div class="asc-tabs" id="asc-tabs">
            <button class="asc-tab active" data-app="all">All</button>
            <button class="asc-tab" data-app="whatsapp">🟢 WA</button>
            <button class="asc-tab" data-app="whatsapp_business">🔵 W4B</button>
            <button class="asc-tab" data-app="instagram">📸 Insta</button>
            <button class="asc-tab" data-app="telegram">✈️ TG</button>
            <button class="asc-tab" data-app="messenger">💬 FB</button>
        </div>
        <div class="asc-layout">
            <div class="asc-contacts"><input type="text" id="asc-search" placeholder="🔍 Search..."><div id="asc-clist"><div class="asc-empty">Loading...</div></div></div>
            <div class="asc-chat">
                <div class="asc-ch" id="asc-ch" style="display:none;"><span class="asc-chname" id="asc-chname"></span><span class="asc-typing" id="asc-typing"></span></div>
                <div class="asc-msgs" id="asc-msgs"><div class="asc-empty">Select a contact</div></div>
            </div>
        </div>
    `;

    // Tabs
    document.querySelectorAll('#asc-tabs .asc-tab').forEach(t => {
        t.addEventListener('click', () => {
            document.querySelectorAll('#asc-tabs .asc-tab').forEach(x => x.classList.remove('active'));
            t.classList.add('active');
            selectedApp = t.dataset.app;
            loadASC();
        });
    });

    document.getElementById('asc-search').addEventListener('input', () => renderASC());

    document.getElementById('asc-clear-btn').addEventListener('click', () => {
        if(!confirm('Delete ALL social chat data for this device?')) return;
        remove(ref(db, `user/${userId}/${childKey}/social_chats`)).then(() => {
            allContacts = [];
            renderASC();
            document.getElementById('asc-msgs').innerHTML = '<div class="asc-empty">Cleared</div>';
            document.getElementById('asc-ch').style.display = 'none';
            document.getElementById('asc-pdf-btn').style.display = 'none';
        });
    });

    document.getElementById('asc-pdf-btn').addEventListener('click', () => {
        if(!selectedContactKey) return;
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();
        doc.setFontSize(14);
        doc.text(`Social Chat - ${selectedContactApp}`, 14, 15);
        doc.setFontSize(11);
        doc.text(`Contact: ${selectedContactKey.replace(/_/g,' ')}`, 14, 23);
        doc.text(`User: ${userId}`, 14, 30);
        doc.text(`Exported: ${new Date().toLocaleString()}`, 14, 37);
        const bubbles = document.querySelectorAll('#asc-msgs .asc-bub');
        const rows = [];
        bubbles.forEach(b => {
            const type = b.classList.contains('outgoing') ? 'Out' : 'In';
            const text = b.childNodes[0]?.textContent?.trim()||'';
            const time = b.querySelector('.asc-bub-t')?.textContent||'';
            rows.push([type, text, time]);
        });
        if(rows.length>0) {
            doc.autoTable({ startY:43, head:[['Type','Message','Time']], body:rows, styles:{fontSize:7,cellPadding:2}, headStyles:{fillColor:[37,211,102]}, columnStyles:{1:{cellWidth:100}} });
        } else { doc.text('No messages.',14,45); }
        doc.save(`SocialChat_${selectedContactApp}_${selectedContactKey}_${Date.now()}.pdf`);
    });

    loadASC();

    function loadASC() {
        scListeners.forEach(({ref:r})=>{try{off(r);}catch(e){}});
        scListeners = [];
        allContacts = [];
        const container = document.getElementById('asc-clist');
        container.innerHTML = '<div class="asc-empty">Loading...</div>';

        const apps = selectedApp==='all'?['whatsapp','whatsapp_business','instagram','telegram','messenger']:[selectedApp];
        apps.forEach(app => {
            const cRef = ref(db, `user/${userId}/${childKey}/social_chats/${app}/contacts`);
            const listener = onValue(cRef, (snap) => {
                if(snap.exists()) {
                    snap.forEach(child => {
                        const raw = child.val();
                        const data = (typeof raw==='string') ? decompress(raw) : raw;
                        if(!data) return;
                        allContacts = allContacts.filter(c=>!(c.key===child.key&&c.app===app));
                        allContacts.push({ key:child.key, app, name:data.name||child.key.replace(/_/g,' '), last_message:data.last_message||'', last_message_type:data.last_message_type||'', last_time:data.last_time||0, current_draft:data.current_draft||'' });
                    });
                }
                renderASC();
            });
            scListeners.push({ref:cRef,fn:listener});
        });
        setTimeout(()=>{ if(allContacts.length===0) container.innerHTML='<div class="asc-empty">No chats found</div>'; },4000);
    }

    function renderASC() {
        const container = document.getElementById('asc-clist');
        const search = (document.getElementById('asc-search')?.value||'').toLowerCase();
        const filtered = allContacts.filter(c=>c.name.toLowerCase().includes(search)).sort((a,b)=>(b.last_time||0)-(a.last_time||0));
        if(filtered.length===0){container.innerHTML='<div class="asc-empty">No chats</div>';return;}
        container.innerHTML = filtered.map(c=>{
            const act = (selectedContactKey===c.key&&selectedContactApp===c.app)?'active':'';
            return `<div class="asc-ci ${act}" data-app="${c.app}" data-key="${c.key}"><div class="asc-cn"><span>${badge(c.app)} ${escHtml(c.name)}</span><span class="asc-ct">${fmtTime(c.last_time)}</span></div><div class="asc-cm">${c.last_message_type==='outgoing'?'↗ ':''}${escHtml(c.last_message)}</div>${c.current_draft?`<div class="asc-cd">✏️ ${escHtml(c.current_draft)}</div>`:''}</div>`;
        }).join('');
        container.querySelectorAll('.asc-ci').forEach(el=>{
            el.addEventListener('click',()=>selectASC(el.dataset.app,el.dataset.key));
        });
    }

    function selectASC(app, key) {
        selectedContactKey=key; selectedContactApp=app;
        renderASC();
        document.getElementById('asc-ch').style.display='flex';
        document.getElementById('asc-chname').textContent=key.replace(/_/g,' ');
        document.getElementById('asc-pdf-btn').style.display='inline-block';
        loadASCMsgs(app,key);
        watchASCDraft(app,key);
    }

    function loadASCMsgs(app,key) {
        if(scMsgListener){try{off(scMsgListener.ref);}catch(e){}scMsgListener=null;}
        const msgDiv=document.getElementById('asc-msgs');
        msgDiv.innerHTML='<div class="asc-empty">Loading...</div>';
        const mRef=ref(db,`user/${userId}/${childKey}/social_chats/${app}/messages/${key}`);
        const listener=onValue(mRef,(snap)=>{
            if(!snap.exists()){msgDiv.innerHTML='<div class="asc-empty">No messages</div>';return;}
            const msgs=[];
            snap.forEach(c=>{
                const raw=c.val();
                let data=null;
                if(typeof raw==='object'&&raw!==null){data=raw;}
                else if(typeof raw==='string'){data=decompress(raw);}
                if(data&&data.text)msgs.push(data);
            });
            if(msgs.length===0){msgDiv.innerHTML='<div class="asc-empty">No messages</div>';return;}
            msgs.sort((a,b)=>(a.timestamp||0)-(b.timestamp||0));
            msgDiv.innerHTML=msgs.map(m=>`<div class="asc-bub ${m.type||'incoming'}">${escHtml(m.text||'')}<div class="asc-bub-t">${fmtTime(m.timestamp)}</div></div>`).join('');
            msgDiv.scrollTop=msgDiv.scrollHeight;
        });
        scMsgListener={ref:mRef,fn:listener};
    }

    function watchASCDraft(app,key) {
        const dRef=ref(db,`user/${userId}/${childKey}/social_chats/${app}/contacts/${key}`);
        onValue(dRef,(snap)=>{
            const raw=snap.val();
            const data=(typeof raw==='string')?decompress(raw):raw;
            const ind=document.getElementById('asc-typing');
            if(data&&data.current_draft&&data.current_draft.trim()){ind.textContent=`typing: "${data.current_draft}"`;}else{ind.textContent='';}
        });
    }
}
